...
content = content.replace(
  /if \(!TMDB_API_KEY\) return page === 1 \? FALLBACK_MOVIES : \[\];/g,
  \`if (!TMDB_API_KEY) {
...
content = content.replace(
  /return page === 1 \? FALLBACK_MOVIES : \[\];/g,
  \`const start = (page - 1) * 20;
      return FALLBACK_MOVIES.slice(start, start + 20);\`
);

content = content.replace(
  /if \(!TMDB_API_KEY\) \{\s*return page === 1 \? FALLBACK_MOVIES\.filter\(m => m\.title\.toLowerCase\(\)\.includes\(query\.toLowerCase\(\)\)\) : \[\];\s*\}/g,
  \`if (!TMDB_API_KEY) {
...
content = content.replace(
  /return page === 1 \? FALLBACK_MOVIES\.filter\(m => m\.title\.toLowerCase\(\)\.includes\(query\.toLowerCase\(\)\)\) : \[\];/g,
  \`const results = FALLBACK_MOVIES.filter(m => m.title.toLowerCase().includes(query.toLowerCase()));
      const start = (page - 1) * 20;
      return results.slice(start, start + 20);\`
);
